...
### ---------------------------------------------------------------------------------------------------------------------
### BacDup options and their accessor and modifier functions.
#########################################################################################################################
#
### G L O B A L S #######################################################################################################
#
...
### @author Yassen Assenov
parse.options <- function(fname.input = "extdata/options.txt") {
 ...
 assign('infos', infos, .bdp.options)
 assign('accepted', accepted, .bdp.options)
 assign('current', current, .bdp.options)
 assign('previous', previous, .bdp.options)
}
parse.options()
...
# bdp.option.compatibility <- function(oname, ovalue) {
...
### @author Yassen Assenov
#bdp.validate.option <- function(oname, ovalue) {
#  infos <- .bdp.options[["infos"]]
#  # ensure backwards compatibility for legacy options
#  ocompat <- bdp.option.compatibility(oname, ovalue)
#  oname   <- ocompat$oname
...
#  ## Validate value(s)
#  if (oname == "assembly") {
#    valid <- ovalue %in% bdp.get.assemblies()
...
#      }
#    }
#    if (oname %in% names(.bdp.options[["accepted"]])) {
#      valid <- isTRUE(all(ovalue %in% .bdp.options[["accepted"]][[oname]]))
#    }
#  }
#  if (!valid) {
#    stop(paste("invalid value for option", oname))
#  }
#  if (oname == "filtering.context.removal") {
#    ovalue <- intersect(.bdp.options[["accepted"]][["filtering.context.removal"]], ovalue)
...
#    .bdp.options[["previous"]][["import.bed.columns"]] <- .bdp.options[["current"]][["import.bed.columns"]]
#    if (ovalue == "Encode") {
#      .bdp.options[["current"]][["import.bed.columns"]] <-
...
###
###  oname    Name of the option to get or set.
###  ovalue   New value for the option. This is used only if setvalue is TRUE.
###  setvalue Flag indicating if the value of the option is to be changed.
###  Yassen Assenov

# bdp.get.option <- function(oname, ovalue = NULL, setvalue = FALSE) {
#  # ensure backwards compatibility for legacy options
#  ocompat <- bdp.option.compatibility(oname, ovalue)
#  oname   <- ocompat$oname
#  ovalue  <- ocompat$ovalue
...
# 
#  if (!(oname %in% names(.bdp.options[["current"]]))) {
#    return(paste(oname, "is invalid option"))
#  }
#  result <- ""
#  cvalue <- .bdp.options[["current"]][[oname]]
#  if (is.null(cvalue)) {
#    .bdp.options[["previous"]][oname] <- list(NULL)
#  } else {
#    .bdp.options[["previous"]][[oname]] <- cvalue
#  }
#  if (setvalue) {
#    ovalue <- tryCatch(bdp.validate.option(oname, ovalue), error = function(e) { e })
#    if (inherits(ovalue, "error")) {
#      result <- ovalue$message
#    } else if (is.null(ovalue)) {
#      .bdp.options[["current"]][oname] <- list(NULL)
#    } else {
#      .bdp.options[["current"]][[oname]] <- ovalue
#    }
#  }
#  return(result)
# }
...
##'
##@seealso \code{\link{bdp.options}} for getting and setting option values
##@examples
##\donttest{
##bdp.is.option("logging") # TRUE
##bdp.is.option("Logging") # FALSE
##}
##@author Yassen Assenov
##@export
#bdp.is.option <- function(txt) {
#  return(is.character(txt) && length(txt) == 1 && txt %in% names(.bdp.options[["current"]]))
#}
#
#########################################################################################################################
#
...
##
##'
## ... Option names as \code{character}s, or new option values given in the form \code{name = value}.
## x   Option name in the form of a \code{character} vector of length 1.
##@return For \code{bdp.getOption}, the current value for \code{x}. For \code{bdp.options()}, a list of all
##        \pkg{RnBeads} options and their current values. If option names are given, a list of all requested options
##        and their values. If option values are set, \code{bdp.options} returns the previous values of the modified
##        options, invisibly.
##'
##@details
##Invoking \code{bdp.options()} with no arguments returns a list with the current values of the options. To access the
##value of a single option, one should use, e.g., \code{bdp.getOption("filtering.greedycut")}, rather than
##\code{bdp.options("filtering.greedycut")} which is a \emph{list} of length one. Also, only a limited set of options
...
##       function \code{\link{bdp.get.assemblies}}.}
...
##       signifies that all available region annotations (as returned by \code{\link{bdp.region.types}}) are summarized
...
##		  See \code{\link{bdp.execute.import}} for further details.}
##  \item{\bold{\code{import.table.separator}}\code{ = ","}}{
##       Separator used in the plain text data tables. See \code{\link{bdp.execute.import}} for details.}
...
##       automatically selected; see \code{\link{bdp.sample.groups}} for how this is done.}
...
##       automatically selected. See \code{\link{bdp.sample.groups}} for how this is done. By default,
...
##		  For more details see \code{\link{bdp.sample.groups}}}
...
##       is presented the Details section of \code{\link{bdp.step.betadistribution}}}.
...
##\donttest{
##str(bdp.options())
##bdp.getOption("filtering.greedycut")
##}
##'
##@author Yassen Assenov
##@export
# bdp.options <- function(...) {
#  optlist <- list(...)
#  if (length(optlist) == 0) {
#    ## Return all options and their current values
#    return(.bdp.options[["current"]])
#  }
...
#      }
#      oresult <- bdp.get.option(oname[1])
#      if (oresult != "") {
#        .bdp.options[["previous"]] <- list()
#        stop(oresult)
#      }
#    }
...
#        oresult <- bdp.get.option(oname)
#      } else {
#        oresult <- bdp.get.option(oname, optlist[[oname]], setvalue = TRUE)
#      }
#      if (oresult != "") {
#        ## An error was reached, rollback
#        if (length(.bdp.options[["previous"]]) != 0) {
#          .bdp.options[["current"]][names(.bdp.options[["previous"]])] <- .bdp.options[["previous"]]
#          .bdp.options[["previous"]] <- list()
#        }
#        stop(oresult)
#      }
...
# bdp.options
#
bdp.getOption <- function(x) {
  if (!(is.character(x) && length(x) == 1 && (!is.na(x)))) {
    stop("invalid option name specified")
  }
  if (!(x[1] %in% names(.bdp.options[["current"]]))) {
    stop(paste(x[1], "is invalid option"))
  }
  return(.bdp.options[["current"]][[x[1]]])
}
...
##@examples
##\donttest{
##cat(bdp.options2xml(), file = "rnbeads_options.xml")
##}
##@author Yassen Assenov
##@export
#bdp.options2xml <- function(pretty = TRUE) {
...
#  NL <- ifelse(pretty, "\n", "")
#  TAB <- ifelse(pretty, "\t", "")
#  o.values <- sapply(bdp.options(), o.value2character)
#  o.values <- paste0(TAB, "<", names(o.values), o.values, "</", names(o.values), ">", collapse = NL)
#  paste0("<bdp.xml>", NL, o.values, NL, "</bdp.xml>", NL)
#}
#
#########################################################################################################################
#
##bdp.performance.profile
##'
...
##@author Pavlo Lutsik
##@export
#bdp.performance.profile<-function(data.type = "450k", profile) {
#  
...
#  invisible(bdp.xml2options(fname))
#}
#
#########################################################################################################################
#
##bdp.options.description.table.fromRd
##'
##Parses the Rd file containing bdp.options descriptions and creates an option description table
##'
##@param rdFile File path of the Rd file (bdp.options.Rd)
##@return A data frame containing option names, descriptions and default settings. row names correspond to option names
##@examples
##\donttest{
##optTab <- bdp.options.description.table.fromRd(file.path("man", "bdp.options.Rd"))
##tabFile <- file.path("inst", "extdata", "option_desc.tsv")
##write.table(optTab, tabFile, sep="\t", quote=FALSE, row.names=FALSE)
##}
##@author Fabian Mueller
##@noRd
#bdp.options.description.table.fromRd <- function(rdFile=file.path("man", "bdp.options.Rd")){
#  bdp.require("tools")
#  bdp.require("XML")
...
##@examples
##\donttest{
##optTab <- bdp.options.description.table()
##str(optTab)
##}
##@author Fabian Mueller
##@noRd
#bdp.options.description.table <- function(){
