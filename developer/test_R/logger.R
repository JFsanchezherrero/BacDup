...
#' ##### Original script https://github.com/epigen/RnBeads/blob/369833af2810371f433909ad202dedaf1fa10e5d/R/logger.R
#' ## ---------------------------------------------------------------------------------------------------------------------
#' ## Logging functionality of the BacDup modules.
#' ########################################################################################################################
#' 
#' ## G L O B A L S #######################################################################################################
...
#' ## logger.format
#' ##
#' ## Memory usage formatter for the BacDup text logger.
#' ##
...
#' #'         (see \code{\link{bdp.options}}) or the logger is not initialized, this function returns \code{NULL}.
#' #'
#' #' @examples
#' #' \donttest{
...
#'   if (bdp.getOption("logging") && exists("logfiles", envir = .LOG.INFO, inherits = FALSE)) {
...
#' #' logger.info(c("Provided email:", bdp.getOption("email")))
#' #' }
#' #' @seealso \code{\link{logger.isinitialized}}to check if logging is activated;
#' #'   \code{\link{logger.start}} for initializing a logger or starting a section
#' #' @author Yassen Assenov
...
#' #' @rdname loggerMessages
#' #' @export
#' logger.error <- function(txt, terminate = bdp.getOption("logging.exit.on.error")) {
#'   if (bdp.getOption("logging")) {
#'     if (!logger.isinitialized()) {
#'       stop("logger is not initialized")
#'       logger.start(fname=NA) #initialize the logger to the console if not started yet
...
# 
#  txt   Description to add to the log file. The words STARTED and COMPLETED are prepended to the
#               message upon initialization and completion of the section, respectively.
#  fname Name of the log file and/or console. Note that at most one file name can be specified. The function
#               logger.start normalizes the given name, that is, it converts it to an absolute name. If this
#               parameter is NA, logger messages are printed to the console. If it is a two-element vector
#               containing one file name and NA, the logger is (re)initialized to print messages both to the
#               given file name and the console. A value of NULL (default) indicates the logger should continue
#               using the previously specified file.

# 
#  section Details:
#  logger.start initializes the logger and/or starts a new section. logger.completed completes the last
#  (innermost) open section in the log. logger.close deinitializes the logger. Note that after reinitialization
...
logger.start <- function(txt = character(0), fname = NULL) {
  if (bdp.getOption("logging")) {
    ...
    if (!is.null(fname)) {
      ...
      .LOG.INFO[["memory"]] <- bdp.getOption("logging.memory")
      .LOG.INFO[["disk"]] <- (bdp.getOption("logging.disk") && bdp.getOption("logging.disk"))
...
#' #'          It expects that logging is enabled (see \code{\link{bdp.options}}). The function fails if this condition is
#' #'          not met.
#' #' @examples
#' #' \donttest{
...
#'   if (bdp.getOption("logging") == FALSE) {
