...
setMethod("initialize", "ReportPlot",
          function(.Object, fname="temp", report=NULL, width=7, height=7, create.pdf=FALSE, low.png=0L, high.png=0L, skip.dev.setup=FALSE) {
            ...
            if (!is.null(report)) {
              .Object@dir.pdf <- bdp.get.directory(report, "pdfs", absolute = TRUE)
              .Object@dir.png.low <- bdp.get.directory(report, "images", absolute = TRUE)
              .Object@dir.png.high <- bdp.get.directory(report, "images-high", absolute = TRUE)
            } else {
...
#
# report.plot   Report plot of interest. This must be an object of type ReportPlot.
...
# parameters create.pdf, low.png and high.png. If create.pdf is TRUE, a PDF device
...
# 
#
#  fname      character vector with one element storing the name of the output file, without the
#                   extension. The initialized object appends .pdf and/or .png to this name.
#  report     Report (object of type Report) to which this plot is going to be added. This
#                   is used to set the directories for PDF and/or PNG files generated for these plots. If this
#                   parameter is NULL, the current working directory is used to host all generated images.
#  width      numeric storing the width of the device in inches. The length of this vector must be
#                   1.
#  height     numeric storing the height of the device in inches. The length of this vector must be
#                   1.
#  create.pdf Flag indicating if a PDF image is to be created. The length of this vector must be 1.
#  low.png    Resolution, in dots per inch, used for the figure image. Set this to 0 or a negative value
#                   to disable the creation of a low resolution image. The length of this vector must be 1}.
#  high.png   Resolution, in dots per inch, used for a dedicated image. Set this to 0} or a negative value
...
# can consist of the following symbols only: Latin letters, digits, dot (.), dash (-) and underline
            ...
            if (handle.errors){
              tryCatch(
                ...
                error=function(ee){
                  ...
                  .Object@ggp <<- bdp.message.plot("plotting error")
                  do.it(.Object)
                }
              )
            } else {
...
# createReportGgPlot
#
# creates a report plot containing a ggplot object. Except for the ggp parameter, the signature and
# behavior is identical to createReportPlot.
# 
#  ggp        ggplot object to be plotted
#  fname      character vector with one element storing the name of the output file, without the
#                   extension. The initialized object appends .pdf and/or .png to this name.
#  report     Report (object of type Report) to which this plot is going to be added. This
#                   is used to set the directories for PDF and/or PNG files generated for these plots. If this
